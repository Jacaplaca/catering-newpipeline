...
function useFetchMealCategory({
    columns,
    sortName,
    sortDirection,
}: {
    columns: TableColumnType[],
    sortName: MealCategorySortName,
    sortDirection: 'asc' | 'desc',
}) {
    const { data: totalCount = 0, refetch: countRefetch, isFetching: countIsFetching }
        = api.specific.mealCategory.count.useQuery({}, {
            enabled: true,
        });
    ...
    const { data: fetchedRows = [], refetch: rowsRefetch, isFetching }
        = api.specific.mealCategory.getMany
            .useQuery({ page, limit, sortName, sortDirection },
                {
                    enabled: true,
                    placeholderData: placeholderData<MealCategory>(limit, columns),
                },
            );
...
export default useFetchMealCategory;
