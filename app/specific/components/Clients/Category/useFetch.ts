...
function useFetchClientCategory({
    columns,
    sortName,
    sortDirection,
}: {
    columns: TableColumnType[],
    sortName: ClientCategorySortName,
    sortDirection: 'asc' | 'desc',
}) {
    const { data: totalCount = 0, refetch: countRefetch, isFetching: countIsFetching }
        = api.specific.clientCategory.count.useQuery({}, {
            enabled: true,
        });
    ...
    const { data: fetchedRows = [], refetch: rowsRefetch, isFetching }
        = api.specific.clientCategory.getMany
            .useQuery({ page, limit, sortName, sortDirection },
                {
                    enabled: true,
                    placeholderData: placeholderData<ClientCategory>(limit, columns),
                },
            );
...
export default useFetchClientCategory;
